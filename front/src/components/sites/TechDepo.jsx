...
const TechDepo = ({scrollRef}) => {
  ...
  sendMail("cargo", {
    ...
    orderNo: newOrder.id,
    from: "info@" + (newOrder.shipping || "cargo") + ".com",
    title: (newOrder.shipping || "") + " Kargo Takip",
    precontent: `${newOrder.shipping} ile gönderiniz yola çıktı!`,
    isFake: false
  }, { delaySeconds: 60 });
  ...
  const handlePayment = () => {
    ...
    if (is3DWaiting) {
      finalizePayment(); // anında çalıştır
      return;
    }
